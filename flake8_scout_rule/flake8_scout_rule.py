...
"""
        ...
        if "flake8_scout_rule" not in config.sections():
            if add_defaults:
                print(
                    "Didn't find a [flake8_scout_rule] section in the flake8 configuration file, "
                    "adding it now."
                )
    ...
    def format_per_file_violations_tracked(  # noqa: CCR001, C901
        ...
        for line in raw_per_file_violations_tracked.split("\n"):
            ...
            match = Flake8ScoutRuleFormatter.TRACKED_VIOLATIONS_LINE_REGEX.match(line)
            if not match:
            ...
            violations_by_count = []
            for code in codes:
                code_match = Flake8ScoutRuleFormatter.CODE_WITH_VIOLATION_COUNT_REGEX.match(code)
                if not code_match:
                    lines_not_parsed.append(line)
                ...
                violations_by_count.append(
                    ViolationsByCount(
                        filename=filename, code=code, count=int(code_match.group("count"))
                    )
                )
