...
class Flake8ScoutRuleFormatter(Default):
    ...
    def add_options(cls, parser: OptionManager) -> None:
        ...
        update_flake8_file_help = (
            "(Default: true) Automatically creates or updates the flake8 configuration file's "
            "[flake8_scout_rule] section with the violations that were found in this run. Meant to "
            "be used in conjunction with having flake8-scout-rule as a plugin for the pre-commit "
            "package."
        )
        parser.add_option(
    ...
    def stop(self):
        ...
        if config_file:
            LOG.debug(f"Found flake8 config file: {config_file}")
        else:
...
""")
        ...
        for z in config['flake8']:
            print(f"Config file flake8: {z}")
